...
Đồ án này tập trung vào việc phân tích toàn diện challenge Runic, một trong những thử thách thuộc mảng Binary Exploitation của cuộc thi. Runic là một chương trình C 64-bit sử dụng ptmalloc (GLIBC malloc implementation) để quản lý dynamic memory, được bảo vệ bởi các cơ chế bảo mật hiện đại như ASLR, NX, Full RELRO, và PIE. Thử thách yêu cầu người chơi phát hiện và khai thác các lỗ hổng trong chương trình và bypass các cơ chế bảo vệ này, cuối cùng đạt được Remote Code Execution và thu thập flag. Đồ án trình bày chi tiết quá trình phân tích từ giai đoạn reconnaissance, phát hiện lỗ hổng, xây dựng exploit primitives, cho đến việc thực thi chuỗi tấn công hoàn chỉnh, kèm theo giải thích sâu về các kỹ thuật heap exploitation được áp dụng và cách thức bypass từng lớp bảo vệ.

\subsection{Lỗ hổng chính}

Lỗ hổng cốt lõi trong Runic nằm ở việc xử lý chuỗi ký tự không đúng cách (improper string handling), cụ thể là trong quá trình tính toán độ dài chuỗi. Khi chương trình sử dụng hàm \texttt{strlen()} để xác định kích thước dữ liệu cần xử lý, nếu dữ liệu đầu vào không được null-terminated hoặc chứa null bytes ở vị trí không mong đợi, \texttt{strlen()} sẽ trả về độ dài sai lệch so với kích thước thực tế của buffer. Điều này dẫn đến tình huống chương trình cấp phát chunk với kích thước nhỏ hơn dữ liệu thực tế sẽ được ghi vào, gây ra heap buffer overflow - một lỗ hổng cho phép ghi dữ liệu vượt quá ranh giới chunk và ghi đè metadata của các chunks liền kề.

Thông qua heap buffer overflow, kẻ tấn công có thể thao túng các metadata quan trọng của ptmalloc như trường \texttt{size} và con trỏ \texttt{fd} trong free chunks, cho phép thực hiện các kỹ thuật khai thác tinh vi như tcache poisoning và unsorted bin exploitation. Việc kiểm soát metadata này, kết hợp với khả năng bypass các cơ chế bảo vệ hiện đại của GLIBC (Safe-Linking, alignment checks, double-free detection), cuối cùng giúp kẻ tấn công đạt được hai primitives mạnh mẽ: Arbitrary Address Write (AAW - khả năng ghi dữ liệu vào địa chỉ bất kỳ) và Arbitrary Address Read (AAR - khả năng đọc dữ liệu từ địa chỉ bất kỳ). Với AAR, kẻ tấn công leak được các địa chỉ quan trọng như libc base và heap base để vượt qua ASLR. Với AAW, kẻ tấn công ghi đè các cấu trúc dữ liệu quan trọng như GOT entries hoặc function pointers, cuối cùng chuyển hướng luồng thực thi của chương trình để đạt được Remote Code Execution (RCE) và thu thập flag.

\subsection{Phương pháp và công cụ}

Phương pháp phân tích chính được sử dụng trong tài liệu này là \textbf{code review} - đọc và phân tích source code để hiểu logic chương trình và xác định các điểm yếu có thể khai thác. Quá trình này được hỗ trợ bởi ba công cụ chính: \textbf{IDA Pro} để thực hiện reverse engineering và phân tích static binary, giúp hiểu rõ cấu trúc chương trình, luồng thực thi và các hàm quan trọng; \textbf{pwndbg} - một plugin mạnh mẽ cho GDB - để debug chương trình trong runtime, quan sát trạng thái heap, kiểm tra giá trị các con trỏ và metadata của chunks, cũng như xác minh các giả thuyết về cách thức hoạt động của exploit; và \textbf{pwntools} - một framework Python được thiết kế đặc biệt cho CTF - để viết exploit script tự động hóa quá trình tấn công, giao tiếp với chương trình target thông qua stdin/stdout, và xử lý các encoding/packing dữ liệu cần thiết.

\subsection{Sử dụng AI trong tài liệu}

\textbf{Claude Code}, một công cụ AI được phát triển bởi Anthropic, được sử dụng để hỗ trợ viết báo cáo này và review pseudocode từ quá trình reverse engineering. Toàn bộ công việc kỹ thuật bao gồm phân tích lỗ hổng, thiết kế và xây dựng exploit, debug, cũng như kiểm thử đều do tác giả tự thực hiện. Việc sử dụng AI được công khai rõ ràng nhằm đảm bảo tính minh bạch và tuân thủ các chuẩn mực học thuật.
...
\subsection{Other Bins}

Ngoài tcache và unsorted bin, ptmalloc còn sử dụng các loại bins khác. \textbf{Fastbins} bao gồm 10 bins dành cho các chunks nhỏ (0x20-0x80 bytes), sử dụng singly-linked list và hoạt động theo LIFO. Fastbins không thực hiện consolidation để tăng tốc allocation/deallocation, và từ GLIBC 2.32 đã áp dụng Safe-Linking tương tự tcache. \textbf{Smallbins} bao gồm 62 bins cho chunks từ 0x20 đến 0x3f0 bytes, mỗi bin chứa chunks có kích thước cố định. \textbf{Largebins} bao gồm 63 bins cho chunks lớn ($\geq$ 0x400 bytes), mỗi bin chứa một khoảng kích thước.
...
\begin{itemize}
    ...
    \item \texttt{Canary found} cho biết chương trình sử dụng stack canaries - một giá trị ngẫu nhiên được đặt trên stack để chống stack buffer overflow trước khi hàm return.
    \item \texttt{NX enabled} đánh dấu các vùng dữ liệu như stack và heap là non-executable, ngăn việc thực thi shellcode trực tiếp tại các vùng này.
